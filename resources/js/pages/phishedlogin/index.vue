...
const props = defineProps({
    ...
    function remove(mailinput) {
        selectedDeleteEntry = mailinput;
        showConfirm.value = true;
    }

    function deleteEntry(){
        deleteForm.delete('/phishedlogin/' + selectedDeleteEntry.id)
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div class="mx-10 my-10 ">
            <div class="div flex items-end justify-between mb-4">
                <div class="div">
                    <div class="h2 font-semibold text-xl text-gray-800 leading-tight">Phished Employees (Account Login)</div>
                    <div class="p text-sm text-gray-500">List of employees who submitted their login credentials during the phishing simulation.</div>
                </div>
            </div>
            ...
            <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="mailinput in mailinputs.data" :key="mailinput.id" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600">
                            ...
                            <td class="px-6 py-4">
                                {{ mailinput.campaigndepartment }}
                            </td>
                            <td class="px-6 py-4">
                                {{ mailinput.clicked_at }}
                            </td>
                            <td class="flex flex-row justify-start ml-6 space-x-2.5 cursor-pointer overflow-hidden">
                                <Link :href="'/phishedlogin/edit/' + mailinput.id">
                                ...
                                <svg @click="remove(mailinput)" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5 hover:scale-110 hover:text-blue-400 my-4.5">
            ...
            <!-- Pagination -->
            <Pagination v-if="mailinputs.data.length>0" :links="mailinputs.links" class="mt-6 flex justify-center"/>
